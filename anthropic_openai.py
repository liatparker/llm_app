    ...
    with c1:
        with st.container(border=True):
            if uploaded_file:  # check if path is not None
                reader = PdfReader(uploaded_file)
                text = ''.join(page.extract_text() for page in reader.pages)
                st.write("summary focused on hypothesis, methodology, results, and conclusions")
                response = get_completion(client=Anthropic(api_key=anthropic_api_key), prompt=(
                f"""Here is an academic paper: <paper>{text}</paper>
                                                       Please do the following:
                                                       1.Write in bullet point form and focus on hypothesis, methodology, results, and conclusions 
    ...
    with c2:
        with st.container(border=True):
            if uploaded_file:  # check if path is not None
                reader = PdfReader(uploaded_file)
                text = ''.join(page.extract_text() for page in reader.pages)
                st.write("summary focused on major sections")
    ...
    with c3:
        with st.container(border=True):
            if uploaded_file:  # check if path is not None
                reader = PdfReader(uploaded_file)
                text = ''.join(page.extract_text() for page in reader.pages)
                st.write("summary focused on  the architecture of the model")
                response3 = get_completion(client=Anthropic(api_key=anthropic_api_key), prompt=(
                f"""Here is an academic paper: <paper>{text}</paper>
                                                           Please do the following:
...
#     txt_input = st.text_input("summary focused on  the architecture of the model ")
